...
<Row>
  <Col>
    ...
    Returns a notebook if a valid note ID was provided.
  </Col>
  <Col sticky>
...
<Row>
  ...
  <Col sticky>
    ...
    ```js
    const db = inkdrop.main.dataStore.getLocalDB()
    const isValid = await db.notes.validateDocId('book:ak0s1ce6')
    ```
...
<Row>
  <Col>
    ...
    Deletes a note with the given notebook ID.
...
<Row>
  ...
  <Col sticky>
    ...
    ```js
    const db = inkdrop.main.dataStore.getLocalDB()
    const num = await db.books.findWithName('Inbox')
    ```

...
Allows commands to be associated with keystrokes in a context-sensitive way. Available as `env.keymaps` on the [Environment](/modules/environment) passed to `activate(env)`.{{ className: 'lead' }}
...
<Row>
  ...
  <Col sticky>
    ...
    ```js
    env.keymaps.onDidMatchBinding((event) => {
...
<Row>
  ...
  <Col sticky>
    <CodeGroup title="Subscription Example">
    ```js
    env.keymaps.onDidPartiallyMatchBindings((event) => {
...
<Row>
  ...
  <Col sticky>
    <CodeGroup title="Subscription Example">
    ```js
    env.keymaps.onDidFailToMatchBinding((event) => {
...
<Row>
  ...
  <Col sticky>
    ...
    ```js
    const bindings = env.keymaps.findKeyBindings({ command: 'editor:new-line' })
    ```
    </CodeGroup>
  </Col>
</Row>

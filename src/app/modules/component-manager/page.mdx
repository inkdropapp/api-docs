...
export const useCustomComponent = <T extends ComponentType<any>>(className: string, DefaultComponent: T) => {
  ...
  const [Component, setComponent] = useState<T>((components.getComponentClass(
    className
  ...
  useEffect(() => {
    ...
    disposables.add(
      components.onClassRegistered<T>(className, (klass) => setComponent(() => klass)),
      components.onClassDeleted(className, () => setComponent(() => DefaultComponent))
    )

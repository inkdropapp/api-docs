...
Available as `env.components` on the [Environment](/modules/environment) passed to `activate(env)`.
...
`getEnv()` is the small helper — shown in the [Word Count plugin walkthrough](/guides/plugin-word-count#understanding-the-generated-code) — that captures the [Environment](/modules/environment) handed to `activate()` so code outside of it, like this hook, can still reach it.

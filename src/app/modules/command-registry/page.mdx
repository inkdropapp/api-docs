...
Available as `env.commands` on the [Environment](/modules/environment) passed to `activate(env)`.{{ className: 'lead' }}
  ...
  <Col sticky>
    ...
    env.commands.add(
      ...
      (event) => {
        ...
        env.store.dispatch(actions.editingNote.update({ body }))
...
<Row>
  ...
  <Col sticky>
    <CodeGroup title="Example">
    ```js
    env.commands.add(document.body, {
      'custom:insert-date': () => {
        const { editingNote } = env.store.getState()
        const body = editingNote.body + '\n' + new Date().toLocaleString()
        env.store.dispatch(actions.editingNote.update({ body }))
...
<Row>
  ...
  <Col sticky>
    ...
    ```js
    env.commands.dispatch(document.body, 'core:save-note')
    ```

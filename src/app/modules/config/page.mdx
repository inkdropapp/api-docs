export const metadata = {
  title: 'Config',
  description: 'This page details how to access and manage Inkdrop\'s configuration settings.',
}

# Config

Used to access all of Inkdrop's configuration details.
This module allows you to get and set configuration settings stored in [`config.json`](https://docs.inkdrop.app/reference/user-data-directory).

Available as `env.config` on the [Environment](/modules/environment) passed to `activate(env)`.

## Getting and setting config settings

```js
// Note that with no value set, `::get` returns the setting's default value.
env.config.get('my-plugin.myKey') // -> 'defaultValue'

env.config.set('my-plugin.myKey', 'value')
env.config.get('my-plugin.myKey') // -> 'value'
```
...
```js
env.config.set('my-plugin.myKey', 'value')
env.config.observe('my-plugin.myKey', newValue => {
...
```js
env.config.onDidChange('my-plugin.myKey', ({ newValue, oldValue }) => {
  console.log('My configuration changed:', newValue, oldValue)
})
```

Alternatively, you can get the config values via the [`localConfig` app state](/states/local-config).
...
env.config.get('my-plugin.anInt') // -> 12

// The string will be coerced to the integer 123
env.config.set('my-plugin.anInt', '123')
env.config.get('my-plugin.anInt') // -> 123

// The string will be coerced to an integer, but it must be greater than 0, so is set to 1
env.config.set('my-plugin.anInt', '-20')
env.config.get('my-plugin.anInt') // -> 1
```
...
See [plugin docs](/guides/plugin-word-count) for more info.
...
```js
let otherSetting = env.config.get('some-plugin.otherSetting')
env.config.set('some-plugin.stillAnotherSetting', otherSetting * 5)
```

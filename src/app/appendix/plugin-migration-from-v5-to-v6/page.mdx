...
`inkdrop.main` was a `remote` reference to the main process, and with it `inkdrop.main.dataStore` is gone. The local database is now an IPC proxy on the environment, with the same `notes` / `books` / `tags` / `files` / `utils` sub-objects and change events — see [Accessing the local database](/guides/access-the-local-database).
...
<Note>
  Returned documents are plain objects now, not Remote Objects, so
  `structuredClone()` is no longer needed. Direct access to the PouchDB instance
  (`inkdrop.main.dataStore.localPouch`) is not available to plugins.
</Note>
...
In v5, diagrams rendered by the [mermaid](https://github.com/inkdropapp/inkdrop-mermaid) plugin had their own `theme` / `themeCSS` / `themeVariables` plugin settings, separate from your theme's stylesheet — matching a diagram's colors to your theme meant configuring the plugin on top of it, if at all.

That plugin setting is gone in v6. Diagrams now always render with a built-in theme driven by CSS variables: `@inkdropapp/css`'s [`mermaid.css`](https://github.com/inkdropapp/css/blob/main/mermaid.css) defines a `--mermaid-*` token for every color Mermaid can draw, layered the same way as the rest of the preview — you override the ones you care about in `preview.css`, and everything else falls back to defaults that already track `--text-color`, `--border-color`, and the rest of your palette. See [Diagram colors (Mermaid)](/guides/create-a-theme#diagram-colors-mermaid) in the theme guide for the token list and an example.

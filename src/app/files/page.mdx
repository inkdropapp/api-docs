...
<Row>
  <Col>
    ...
    <Properties>
      <Property name="static loadWithId(fileId: string)">
        Loads a file by its ID from the local database and returns a new instance of the `File`.
      </Property>
      <Property name="constructor(initialValues: Partial<File>)">
...
<Row>
  ...
  <Col sticky>
    ...
    const file = await db.files.get('file:tBELDUITs')
    ```
...
## Lookup a file by name {{ label: 'findWithName(name)' }}
...
<Row>
  <Col>
    ...
    Returns a file if found otherwise `undefined`.
  </Col>
  <Col sticky>
    ...
    ```js
    const db = inkdrop.main.dataStore.getLocalDB()
    const file = await db.files.findWithName('Videography')
    ```

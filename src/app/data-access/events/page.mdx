...
<Row>
  ...
  <Col sticky>
    ...
    ```js
    const db = env.localDB
    const subscription = db.onFullTextIndexBuildStart(() => {
...
<Row>
  ...
  <Col sticky>
    ...
    ```js
    const db = env.localDB
    const subscription = db.onFullTextIndexBuildEnd(() => {
...
<Row>
  ...
  <Col sticky>
    ...
    ```js
    const db = env.localDB
    const subscription = db.onFullTextIndexBuildError(error => {
      console.log('Full-text search index building has failed', error)
    });
